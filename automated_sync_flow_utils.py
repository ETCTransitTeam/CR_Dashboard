...
def clean_route_code(route_code_series):
    """Clean route codes by removing _00, _01, etc. suffixes"""
    return (
        route_code_series
        .astype(str)
        .str.replace(r'_\d{2}$', '', regex=True)  # Remove _00, _01, _02, etc.
        .str.strip()
    )
...
def calculate_avg_time(time_series):
    """Calculate average time from HH:MM:SS formatted series, filtering out non-time values"""
    # Filter out non-time values (like percentages, 'N/A', etc.)
    valid_times = time_series[
        time_series.str.match(r'^\d{2}:\d{2}:\d{2}$', na=False) & 
        (time_series != "00:00:00")
    ]
    
    if len(valid_times) == 0:
        return "00:00:00"
    
    total_seconds = valid_times.apply(
...
def process_surveyor_data_transit_ls6(ke_df, elvis_df, project=None, race_label_map=None):
    """Process data for surveyor-level report
    
    Args:
        ke_df: DataFrame with survey data (kingelvis)
...
def process_route_data_transit_ls6(df, elvis_df, race_label_map=None):
    ...
    if len(existing_time_cols) >= 2:
        ...
        if time_columns_to_calc:
            ...
            for col in time_columns_to_calc:
                survey_time_group[col] = survey_time_group[col].apply(
                    lambda x: "00:00:00" if pd.isna(x) or x < 0 else 
                    f"{int(x//3600):02}:{int((x%3600)//60):02}:{int(x%60):02}"
                )
            
            route_report_df = route_report_df.merge(survey_time_group, how='left')

    # Fill missing time columns with default values
    for col in ['SurveyTime (All)', 'SurveyTime (TripLogic)', 'SurveyTime (DemoLogic)']:
    ...
    if contest_columns_exist:
        ...
        route_report_df['% of Contest - Yes'] = 'N/A'
        route_report_df['% of Contest - (Yes & Good Info)/Overall # of Records'] = 'N/A'
    ...
    # Add time averages - ONLY for actual time columns
    time_columns = ['SurveyTime (All)', 'SurveyTime (TripLogic)', 'SurveyTime (DemoLogic)']
    for time_col in time_columns:
        if time_col in route_report_df.columns:
            # Only calculate average for non-total rows and ensure they are time strings
            non_total_rows = route_report_df[route_report_df['ROUTE_ROOT'] != 'Total']
            time_values = non_total_rows[time_col]
            total_row[time_col] = calculate_avg_time(time_values)
        else:
            total_row[time_col] = "00:00:00"

    # Add average percentages
    percent_cols = [col for col in route_report_df.columns if col.startswith('% of')]
    for col in percent_cols:
        if col in route_report_df.columns:
            # Filter out non-percentage values like 'N/A' or 'Error'
            values = route_report_df.loc[route_report_df['ROUTE_ROOT'] != 'Total', col]
            numeric_values = [float(v.rstrip('%')) for v in values if v.endswith('%') and v != 'N/A' and v != 'Error']
            if numeric_values:
                avg = sum(numeric_values) / len(numeric_values)
                total_row[col] = f"{avg:.1f}%"
            else:
                total_row[col] = '0.0%'
        else:
            total_row[col] = '0.0%'
    ...
    # Define column order dynamically based on what exists
    base_columns = [
        'Route', '# of Records', '# of Supervisor Delete', '# of Records Remove',
        '# of Records Reviewed', '# of Records Not Reviewed', 'SurveyTime (All)',
        'SurveyTime (TripLogic)', 'SurveyTime (DemoLogic)', '% of Incomplete Home Address'
    ...
    # Add dynamic columns that actually exist
    existing_columns = [col for col in base_columns if col in route_report_df.columns]
    
    # Add other percentage columns
    other_percent_cols = [col for col in route_report_df.columns if col.startswith('% of') and col not in existing_columns]
    
    final_columns = existing_columns + other_percent_cols
...
def process_surveyor_date_data_transit_ls6(
    ...
    summary_df = (
        ...
        .merge(record_counts, on='Date_Surveyor', how='left')
        .fillna(0)
    )
    ...
    # -------------------------------
    # 6. ADDRESS COMPLETENESS (ELVIS)
    # -------------------------------
    address_fields = [
        'HOME_ADDRESS_LAT', 'HOME_ADDRESS_LONG', 'HOME_ADDRESS_PLACE',
        'HOME_ADDRESS_ADDR', 'HOME_ADDRESS_CITY',
        'HOME_ADDRESS_STATE', 'HOME_ADDRESS_ZIP'
    ]

    def is_incomplete(row):
        for col in address_fields:
            if col in row and (pd.isna(row[col]) or str(row[col]).strip() in ['', '-']):
                return True
        return False

    filtered_elvis['Incomplete_Address'] = filtered_elvis.apply(is_incomplete, axis=1)

    address_group = filtered_elvis.groupby('Date_Surveyor').agg(
        total_records=('id', 'count'),
        incomplete_count=('Incomplete_Address', 'sum')
    ).reset_index()
    ...
    # -------------------------------
    # 7. SURVEY TIMES (ELVIS)
    # -------------------------------
    for col in ['HOMEADD_TIME', 'NOTE_TIME', 'REVIEWSCR_TIME']:
        filtered_elvis[col] = pd.to_datetime(filtered_elvis[col], errors='coerce')
    ...
    survey_time_group = filtered_elvis.groupby('Date_Surveyor').agg({
        'SurveyTime (All)': 'mean',
        'SurveyTime (TripLogic)': 'mean',
        'SurveyTime (DemoLogic)': 'mean'
    }).reset_index()

    for col in survey_time_group.columns:
        if col != 'Date_Surveyor':
            survey_time_group[col] = survey_time_group[col].apply(
                lambda x: "00:00:00" if pd.isna(x) or x < 0
                else f"{int(x//3600):02}:{int((x%3600)//60):02}:{int(x%60):02}"
            )

    summary_df = summary_df.merge(survey_time_group, how='left').fillna("00:00:00")

    # -------------------------------
    # 8. OTHER METRICS (ELVIS)
    # -------------------------------
    metrics = [
        ('0 Transfers',
         (filtered_elvis['PREV_TRANSFERS'] == '(0) None') &
    ...
    total_records = (
        filtered_elvis.groupby('Date_Surveyor')['id']
        .count()
    ...
    for name, condition in metrics:
        ...
        percent_df = total_records.merge(count_df, on='Date_Surveyor', how='left').fillna(0)
        percent_df[f'% of {name}'] = (
            percent_df['cnt'] / percent_df['total_records'] * 100
        ).apply(format_percentage)

        summary_df = summary_df.merge(
            percent_df[['Date_Surveyor', f'% of {name}']],
            how='left'
        ).fillna('0.0%')
    ...
    # Keep Date_Surveyor as string for filtering (convert Date back to string format)
    # Handle 'Total' row specially
    mask = summary_df['Date_Surveyor'] != 'Total'
    # Ensure consistent date format (YYYY-MM-DD) when reconstructing Date_Surveyor
    # Convert Date to string in YYYY-MM-DD format for consistent filtering
    def format_date_for_key(date_val):
        """Format date consistently as YYYY-MM-DD string"""
        if pd.isna(date_val) or date_val == 'Total':
            return str(date_val)
        try:
            # Convert to datetime first to handle all formats
            if isinstance(date_val, str):
                parsed = pd.to_datetime(date_val, errors='coerce')
            elif hasattr(date_val, 'strftime'):
                # It's a date/datetime object - convert to string first
    ...
    # Reconstruct Date_Surveyor with consistent date format
    summary_df.loc[mask, 'Date_Surveyor'] = (
        summary_df.loc[mask, 'Date'].apply(format_date_for_key) + '_' + 
        summary_df.loc[mask, 'INTERV_INIT'].astype(str)
    )
...
def process_route_date_data_transit_ls6(
    ke_df,
    elvis_df,
    survey_date_route,
    race_label_map=None
    ...
    """
    Date-filtered version of process_route_data_transit_ls6.
    Logic is IDENTICAL to original, grouping key is Date_Route.
    ...
    route_report_df = (
        ...
        .merge(record_counts, on='Date_Route', how='left')
        .fillna(0)
    )
    ...
    route_report_df = route_report_df.merge(
        delete_counts, on='Date_Route', how='left'
    ).fillna(0)
    ...
    existing_address_fields = [
        c for c in address_fields if c in filtered_elvis.columns
    ]

    if existing_address_fields:
        filtered_elvis['Incomplete_Address'] = filtered_elvis.apply(
            lambda r: any(
                pd.isna(r[c]) or str(r[c]).strip() in ['', '-']
                for c in existing_address_fields
            ),
            axis=1
        )
        ...
        address_group['% of Incomplete Home Address'] = (
            address_group['incomplete_count'] /
            address_group['total_records'] * 100
        ).apply(lambda x: f"{x:.1f}%")
    ...
    # -------------------------------
    # 7. SURVEY TIME METRICS
    # -------------------------------
    time_cols = ['HOMEADD_TIME', 'NOTE_TIME', 'REVIEWSCR_TIME']
    time_cols = [c for c in time_cols if c in filtered_elvis.columns]

    for col in time_cols:
        filtered_elvis[col] = pd.to_datetime(filtered_elvis[col], errors='coerce')

    if {'HOMEADD_TIME','NOTE_TIME'}.issubset(time_cols):
        filtered_elvis['SurveyTime (All)'] = (
            filtered_elvis['NOTE_TIME'] - filtered_elvis['HOMEADD_TIME']
        ).dt.total_seconds()

    if {'HOMEADD_TIME','REVIEWSCR_TIME'}.issubset(time_cols):
        filtered_elvis['SurveyTime (TripLogic)'] = (
            filtered_elvis['REVIEWSCR_TIME'] - filtered_elvis['HOMEADD_TIME']
        ).dt.total_seconds()

    if {'REVIEWSCR_TIME','NOTE_TIME'}.issubset(time_cols):
        filtered_elvis['SurveyTime (DemoLogic)'] = (
            filtered_elvis['NOTE_TIME'] - filtered_elvis['REVIEWSCR_TIME']
        ).dt.total_seconds()

    survey_time_cols = [
        c for c in filtered_elvis.columns if c.startswith('SurveyTime')
    ]

    if survey_time_cols:
        survey_time_group = (
            filtered_elvis
            .groupby('Date_Route')[survey_time_cols]
            .mean()
            .reset_index()
        )

        for c in survey_time_cols:
            survey_time_group[c] = survey_time_group[c].apply(
                lambda x: "00:00:00"
                if pd.isna(x) or x < 0
                else f"{int(x//3600):02}:{int((x%3600)//60):02}:{int(x%60):02}"
            )
        ...
        route_report_df = route_report_df.merge(
            survey_time_group, on='Date_Route', how='left'
        )
    ...
    # -------------------------------
    # 8. OTHER METRICS (UNCHANGED)
    # -------------------------------
    metrics = []
    add_race_metrics_to_list(metrics, filtered_elvis, race_label_map)
    ...
    total_records_group = (
        ...
        .count()
        .reset_index()
        .rename(columns={'id': 'total_records'})
    )
    ...
    for name, condition in metrics:
        ...
        percent_df = (
            total_records_group
            .merge(metric_group, on='Date_Route', how='left')
            .fillna(0)
        )
        ...
        route_report_df = route_report_df.merge(
            percent_df[['Date_Route', f'% of {name}']],
            on='Date_Route',
            how='left'
        ).fillna('0.0%')
    ...
    base_columns = [
        'Date','ROUTE_ROOT','# of Records','# of Supervisor Delete',
        '# of Records Remove','# of Records Reviewed',
        '# of Records Not Reviewed','SurveyTime (All)',
        'SurveyTime (TripLogic)','SurveyTime (DemoLogic)',
        '% of Incomplete Home Address'
    ]

    other_cols = [c for c in route_report_df.columns if c.startswith('% of')]

    return route_report_df[base_columns + other_cols]
...
def process_reverse_direction_logic(wkday_overall_df, df, route_level_df, project_name, stops_df=None):
    ...
    def get_route_name_from_code(route_code):
        ...
        route_code_str = str(route_code).strip()
        
        # TRY 1: Look in the original df first - exact match
        matching_rows = df[df[route_survey_column[0]] == route_code_str]
        if not matching_rows.empty and route_survey_name_column[0] in matching_rows.columns:
            name = matching_rows.iloc[0][route_survey_name_column[0]]
            if pd.notna(name) and str(name).strip() != '':
                return str(name).strip()
        
        # TRY 2: Try to match without direction suffix
        if '_' in route_code_str:
            # Remove the last part (direction code like _00, _01, etc.)
            parts = route_code_str.split('_')
            if len(parts) > 1:
                # Try different combinations
                base_route_options = []
                
                # Option 1: Remove just the last part (direction)
                base_route_options.append('_'.join(parts[:-1]))
                
                # Option 2: If the last part is a direction code (00, 01, 02, 03)
                last_part = parts[-1]
                if last_part in ['00', '01', '02', '03']:
                    # Remove direction code
                    base_route_options.append('_'.join(parts[:-1]))
                
                for base_route in base_route_options:
                    # Look for base route in original df
        ...
        # TRY 3: Look in transfer route columns from original df
        ...
        for code_col, name_col in zip(transfer_code_columns, transfer_name_columns):
            if code_col in df.columns and name_col in df.columns:
                ...
                # Also try without direction suffix
                if '_' in route_code_str:
        ...
        # TRY 4: Check in reverse_df's original route columns
        # FIX: reverse_df is in closure, not locals(), so access directly with try/except
        try:
            # Check ORIGINAL_ROUTE_SURVEYEDCode
            if 'ORIGINAL_ROUTE_SURVEYEDCode' in reverse_df.columns and 'ORIGINAL_ROUTE_SURVEYED' in reverse_df.columns:
                matching_rows = reverse_df[reverse_df['ORIGINAL_ROUTE_SURVEYEDCode'] == route_code_str]
                if not matching_rows.empty:
                    name = matching_rows.iloc[0]['ORIGINAL_ROUTE_SURVEYED']
                    if pd.notna(name) and str(name).strip() != '':
                        return str(name).strip()
            
            # Check current route surveyed
            if route_survey_column[0] in reverse_df.columns and route_survey_name_column[0] in reverse_df.columns:
                matching_rows = reverse_df[reverse_df[route_survey_column[0]] == route_code_str]
                if not matching_rows.empty:
                    name = matching_rows.iloc[0][route_survey_name_column[0]]
                    if pd.notna(name) and str(name).strip() != '':
                        return str(name).strip()
        except (NameError, AttributeError):
            # If reverse_df doesn't exist yet (early in function execution)
            pass
        
        # TRY 5: Check wkday_overall_df for CR route info
        # FIX: wkday_overall_df is in closure, not locals(), so access directly
        if wkday_overall_df is not None and 'LS_NAME_CODE' in wkday_overall_df.columns and 'LS_NAME' in wkday_overall_df.columns:
            matching_rows = wkday_overall_df[wkday_overall_df['LS_NAME_CODE'] == route_code_str]
            if not matching_rows.empty:
                name = matching_rows.iloc[0]['LS_NAME']
                if pd.notna(name) and str(name).strip() != '':
                    return str(name).strip()
    ...
    def resolve_final_direction_code(route_code, record_id=None, route_type=None):
        ...
        # 2. CR mapping
        if route_code in cr_route_info:
            base = cr_route_info[route_code].get('BASE_ROUTE_CODE', '')
            dir_code = cr_route_info[route_code].get('FINAL_DIRECTION_CODE', '')
            if base and dir_code:
                return f"{base}_{dir_code}"

        # 3. Already directioned
        if isinstance(route_code, str) and route_code.endswith(('00', '01', '02', '03')):
            return route_code
    ...
    def fill_missing_final_direction_codes():
        ...
        and add FINAL_DIRECTION_NAME by matching FINAL_DIRECTION_CODE with ETC_STOP_ID in stops_df
        """
        
        # Step 1: Fill empty FINAL_DIRECTION_CODE with original route code
        def fill_empty_direction_codes(df_to_fill):
            filled_count = 0
            for index, row in df_to_fill.iterrows():
                ...
                # Fill empty FINAL_DIRECTION_CODE with original route code
                if not current_final_direction_code or current_final_direction_code == '' or pd.isna(current_final_direction_code):
                    if original_route_code and pd.notna(original_route_code) and str(original_route_code).strip() != '':
                        df_to_fill.loc[index, 'FINAL_DIRECTION_CODE'] = original_route_code
                        filled_count += 1
            return filled_count
        
        # Fill empty direction codes first
        fill_empty_direction_codes(reverse_df)
        fill_empty_direction_codes(all_type_df)
        
        # Step 2: Build mapping from ETC_ROUTE_ID to ETC_ROUTE_NAME using stops_df
        route_id_to_route_name = {}
        if stops_df is not None and not stops_df.empty:
            if 'ETC_ROUTE_ID' in stops_df.columns and 'ETC_ROUTE_NAME' in stops_df.columns:
                for index, row in stops_df.iterrows():
                    route_id = row.get('ETC_ROUTE_ID', '')
                    route_name = row.get('ETC_ROUTE_NAME', '')
                    if pd.notna(route_id) and pd.notna(route_name) and str(route_id).strip() != '' and str(route_name).strip() != '':
                        ...
                        if route_id_str not in route_id_to_route_name:
                            route_id_to_route_name[route_id_str] = route_name_str
        
        # Step 3: Also build fallback mapping from ROUTE_SURVEYEDCode to ROUTE_SURVEYED
        route_code_to_name = {}
        
        # Source 1: Original df - ROUTE_SURVEYEDCode to ROUTE_SURVEYED mapping
        if route_survey_column and route_survey_name_column and len(route_survey_column) > 0 and len(route_survey_name_column) > 0:
        ...
        # Source 2: reverse_df - ROUTE_SURVEYEDCode to ROUTE_SURVEYED mapping
        if route_survey_column and route_survey_name_column and len(route_survey_column) > 0 and len(route_survey_name_column) > 0:
        ...
        # Source 3: all_type_df - ROUTE_SURVEYEDCode to ROUTE_SURVEYED mapping
        if route_survey_column and route_survey_name_column and len(route_survey_column) > 0 and len(route_survey_name_column) > 0:
            for index, row in all_type_df.iterrows():
                route_code = row.get(route_survey_column[0], '')
                route_name = row.get(route_survey_name_column[0], '')
                if pd.notna(route_code) and pd.notna(route_name) and str(route_code).strip() != '' and str(route_name).strip() != '':
                    route_code_to_name[str(route_code).strip()] = str(route_name).strip()
        
        # Step 4: Fill FINAL_DIRECTION_NAME by matching FINAL_DIRECTION_CODE with ETC_STOP_ID in stops_df
        def fill_direction_names(df_to_fill, df_name):
            """Fill FINAL_DIRECTION_NAME by matching FINAL_DIRECTION_CODE with ETC_STOP_ID in stops_df"""
            # Initialize FINAL_DIRECTION_NAME column if it doesn't exist
            if 'FINAL_DIRECTION_NAME' not in df_to_fill.columns:
                df_to_fill['FINAL_DIRECTION_NAME'] = ''
            ...
            for index, row in df_to_fill.iterrows():
                ...
                current_final_direction_name = row.get('FINAL_DIRECTION_NAME', '')
                
                # Skip if Type is empty
                if not current_type or current_type == '':
                    continue
                
                # Skip if FINAL_DIRECTION_CODE is empty
                if not final_direction_code or final_direction_code == '' or pd.isna(final_direction_code):
                    continue
                
                # Only fill if FINAL_DIRECTION_NAME is empty or missing
                if not current_final_direction_name or current_final_direction_name == '' or pd.isna(current_final_direction_name):
                    direction_code_str = str(final_direction_code).strip()
                    
                    # Skip if invalid
                    if direction_code_str in ['', 'nan', 'None', 'NaT']:
                        continue
                    
                    route_name = ''
                    
                    # PRIMARY STRATEGY: Match FINAL_DIRECTION_CODE with ETC_ROUTE_ID in stops_df
                    if stops_df is not None and not stops_df.empty and 'ETC_ROUTE_ID' in stops_df.columns and 'ETC_ROUTE_NAME' in stops_df.columns:
                        # Strategy 1: Direct match - FINAL_DIRECTION_CODE matches ETC_ROUTE_ID exactly
                        route_name = route_id_to_route_name.get(direction_code_str, '')
                        
                        # Strategy 2: Try without direction suffix (e.g., "ACT_2_51B_00" -> "ACT_2_51B")
                        if not route_name and '_' in direction_code_str:
                            parts = direction_code_str.split('_')
                            if len(parts) > 1 and parts[-1] in ['00', '01', '02', '03']:
                                base_route = '_'.join(parts[:-1])
                                # Try exact match with base route
                                route_name = route_id_to_route_name.get(base_route, '')
                    
                    # FALLBACK STRATEGIES: If stops_df matching didn't work, try other sources
                    # Strategy 4: Direct match - FINAL_DIRECTION_CODE matches ROUTE_SURVEYEDCode
                    if not route_name:
                        route_name = route_code_to_name.get(direction_code_str, '')
                    
                    # Strategy 5: Try without direction suffix in route_code_to_name
                    if not route_name and '_' in direction_code_str:
                        parts = direction_code_str.split('_')
                        if len(parts) > 1 and parts[-1] in ['00', '01', '02', '03']:
                            base_route = '_'.join(parts[:-1])
                    ...
                    # Strategy 6: Check in the same row - if ROUTE_SURVEYEDCode matches FINAL_DIRECTION_CODE
                    if not route_name and route_survey_column and len(route_survey_column) > 0:
                        if route_survey_column[0] in row:
                            row_route_code = str(row[route_survey_column[0]]).strip()
                            if row_route_code == direction_code_str and route_survey_name_column and len(route_survey_name_column) > 0:
                                if route_survey_name_column[0] in row:
                                    route_name = row[route_survey_name_column[0]]
                                    if pd.notna(route_name) and str(route_name).strip() != '':
                                        route_name = str(route_name).strip()
                    
                    # Strategy 7: Use ORIGINAL_ROUTE_SURVEYED if FINAL_DIRECTION_CODE matches ORIGINAL_ROUTE_SURVEYEDCode
                    if not route_name and 'ORIGINAL_ROUTE_SURVEYEDCode' in row and 'ORIGINAL_ROUTE_SURVEYED' in row:
                        original_code = str(row['ORIGINAL_ROUTE_SURVEYEDCode']).strip()
                        if original_code == direction_code_str:
                    ...
                    if route_name:
                        ...
                        if direction_code_str not in ['', 'nan', 'None', 'NaT']:
                            print(f"DEBUG: Could not find name for FINAL_DIRECTION_CODE: {direction_code_str} (Type: {current_type}, ID: {row.get('id', 'N/A')})")
            
            return names_added, missing_names_count
        
        # Fill names for both dataframes
        reverse_names, reverse_missing = fill_direction_names(reverse_df, "reverse_df")
        all_type_names, all_type_missing = fill_direction_names(all_type_df, "all_type_df")
    ...
    # Filter out records where reverse trips are already generated
    reverse_df = reverse_df[reverse_df['REVERSE_TRIPS_STATUS'] != 'Already Generated'].copy()
    ...
    def update_route_surveyed_info(target_index, route_code, route_name, assigned_type, original_row):
        ...
        reverse_df.loc[target_index, 'FINAL_DIRECTION_CODE'] = resolve_final_direction_code(
            route_code,
            record_id=original_row['id'],
            route_type=assigned_type
        )
