...
def _safe_read_flag_csv(path: str) -> pd.DataFrame:
    """Read a flag CSV; treat missing/empty files as zero flags (continue pipeline)."""
    try:
        if not os.path.exists(path):
            print(f"Flag CSV not found (treating as zero flags): {path}")
            return pd.DataFrame(columns=["id"])
        df = pd.read_csv(path)
        if df.empty:
            print(f"Flag CSV has zero rows (treating as zero flags): {path}")
        return df
    except pd.errors.EmptyDataError:
        print(f"Flag CSV is empty (treating as zero flags): {path}")
        return pd.DataFrame(columns=["id"])
...
traditional_df = _safe_read_flag_csv(
    f'reviewtool_{today_date}_{project_name}_TraditionalTransferFlags_auto_approved.csv'
)
od_df = _safe_read_flag_csv(
    f'reviewtool_{today_date}_{project_name}_OD_Distance_Checks_auto_approved.csv'
)
transfer_df = _safe_read_flag_csv(
    f'reviewtool_{today_date}_{project_name}_Distance_Transfer_Flags_auto_approved.csv'
)
try:
    stoplist_df = pd.read_excel(f'{project_name}_flagged_stops_{today_date}_auto_approved.xlsx')
except Exception:
...
def generate_flag_insights(df):
    ...
    if od_cols:
        ...
        od_grouped = od_flags.copy()
        od_grouped['FINAL_REVIEWER'] = df['FINAL_REVIEWER']
        top_surveyors = od_grouped.groupby('FINAL_REVIEWER')[od_cols].sum().sum(axis=1).sort_values(ascending=False).head(5)
    ...
    if trad_cols:
        ...
        trad_grouped = trad_flags.copy()
        trad_grouped['ROUTE_SURVEYEDCode'] = df['ROUTE_SURVEYEDCode']
        top_routes = trad_grouped.groupby('ROUTE_SURVEYEDCode')[trad_cols].sum().sum(axis=1).sort_values(ascending=False).head(5)
    ...
    if trans_cols:
        ...
        trans_grouped = trans_flags.copy()
        trans_grouped['FINAL_REVIEWER'] = df['FINAL_REVIEWER']
        top_transfers = trans_grouped.groupby('FINAL_REVIEWER')[trans_cols].sum().sum(axis=1).sort_values(ascending=False).head(5)
    ...
    if stop_cols:
        ...
        stop_grouped = stop_flags.copy()
        stop_grouped['FINAL_REVIEWER'] = df['FINAL_REVIEWER']
        top_stops = stop_grouped.groupby('FINAL_REVIEWER')[stop_cols].sum().sum(axis=1).sort_values(ascending=False).head(5)
...
def generate_analysis_report(merged_df, project_name, today_date):
    ...
    # If a record is flagged, clear Final_Usage and FINAL_REVIEWER unless reviewer is Field Approved
    # (flags still show 1 in flag columns; Field Approved rows keep Use/reviewer intact).
    flagged_mask = merged_df['SUM_ALL_CHECKS'] == 1
    if 'FINAL_REVIEWER' in merged_df.columns:
        _r = merged_df['FINAL_REVIEWER'].fillna('').astype(str).str.strip().str.lower()
        field_approved_mask = _r == 'field approved'
    else:
        field_approved_mask = pd.Series(False, index=merged_df.index)
    clear_usage_and_reviewer = flagged_mask & ~field_approved_mask
    if 'Final_Usage' in merged_df.columns:
        merged_df.loc[clear_usage_and_reviewer, 'Final_Usage'] = ''
    if 'FINAL_REVIEWER' in merged_df.columns:
        merged_df.loc[clear_usage_and_reviewer, 'FINAL_REVIEWER'] = ''

    # --- Stop Validation Insights ---
    stop_csv_file = f'{project_name}_flagged_stops_{today_date}_auto_approved.xlsx'
    ...
    od_csv_file = f'reviewtool_{today_date}_{project_name}_OD_Distance_Checks_auto_approved.csv'
    ...
    trad_csv_file = f'reviewtool_{today_date}_{project_name}_TraditionalTransferFlags_auto_approved.csv'
    ...
    transfer_csv_file = f'reviewtool_{today_date}_{project_name}_Distance_Transfer_Flags_auto_approved.csv'
    if os.path.exists(transfer_csv_file):
        try:
            transfer_df = pd.read_csv(transfer_csv_file)

            # If file has headers but no rows
            if transfer_df.empty:
                print("⚠️ Transfer file is empty → using zero values")
                transfer_df = pd.DataFrame({'dummy': [0]})

        except pd.errors.EmptyDataError:
            print("⚠️ Transfer file completely empty → using zero values")
            transfer_df = pd.DataFrame({'dummy': [0]})
...
merged_df.to_csv(f'reviewtool_{today_date}_{project_name}_combinedflags_auto_approved.csv',index=False)
