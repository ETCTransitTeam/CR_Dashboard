...
# details_df=pd.read_excel("details_project_od_excel_COTA (1).xlsx",sheet_name='AIR')
...
# details_df=pd.read_excel("details_2023_GCRTA_od_excel (1).xlsx",sheet_name='AIR')
# df=pd.read_csv('elvisawsclevelandrta2023obweekday_export_odbc(new3).csv')

# elvis_df=pd.read_excel('BCT_KINGElvis.xlsx',sheet_name='Elvis_Review')
# file_name = 'COTA_KINGElvis (4).xlsx'
# details_df=pd.read_excel("details_project_od_excel_COTA (1).xlsx",sheet_name='STOPS')
...
# Load the CSV file
file_name = f'reviewtool_{today_date}_{project_name}_OD_Distance_Checks.csv'
df = pd.read_csv(file_name)

# Filter rows where SUM_ALL_CHECKS equals 1
flagged_df = df[df['SUM_ALL_CHECKS'] == 1]
...
reasons_counts = flagged_df[specified_columns].apply(lambda x: (x == 1).sum()).to_dict()
